...
/*
Changelog
mkemper@marc-kemper.com - 2007/09/25 - Replace all htmlspecialchars with htmlentities
mkemper@marc-kemper.com - 2007/09/25 - Add htmlentities to server/channel/subchannel-topics
mkemper@marc-kemper.com - 2007/09/25 - Add new serverpropertie "showplayerstatus"
mkemper@marc-kemper.com - 2007/09/25 - Add new serverpropertie "maxdisplaylength"
*/
...
class teamspeakDisplayClass {
	...
	function _setPlayerDisplayName(&$playerInfo, $withStatus) {
		...
		$playerInfo["displayname"] = $playerInfo["playername"] . (($withStatus) ? (" (" . $playerstatus . ")"):("")) . $playerattributes;
	}
	
	function _getPlayerList($socket, $withStatus) {
		...
		do {
			...
			if (($buffer != "OK") && (strtoupper(substr($buffer, 0, 5)) != "ERROR")) {
				...
				$this->_setPlayerDisplayName($result[$playerid], $withStatus);
			}
		} while (($buffer != "OK") && (strtoupper(substr($buffer, 0, 5)) != "ERROR") && (!feof($socket)));
		return $result;
	}
	
	function _getLimitedPlayerList($socket, $channelList, $withStatus) {
		$playerList = $this->_getPlayerList($socket, $withStatus);
		$result = array();
	...
	function queryTeamspeakServerEx($settings) {
		...
		if (! $this->_openConnection($socket, $settings["serveraddress"], $settings["serverqueryport"], 0.3)) {
			...
			$result["playerlist"] = ($settings["limitchannel"] == "") ? $this->_getPlayerList($socket, $settings['showplayerstatus']) : $this->_getLimitedPlayerList($socket, $result["channellist"], $settings['showplayerstatus']);
			$this->_closeConnection($socket);
		}
		return $result;
	}
	...
	function getDefaultSettings() {
		$result = array();
		$result["serveraddress"] = "localhost";
		$result["serverudpport"] = 8767;
		$result["serverqueryport"] = 51234;
		$result["limitchannel"] = "";
		$result["forbiddennicknamechars"] = "()[]{}";
		$result["showplayerstatus"] = true;
		$result["maxdisplaylength"] = 0;
		return $result;
	}
	...
	function displayTeamspeakEx($settings) {
		...
		$host_url = $settings['teamspeak_host_url'];
		...
		if ($serverInfo["queryerror"] != 0) {
			...
			echo("<table><tr><td>");
			echo("<img src=\"$host_url/teamspeakdisplay/teamspeak_offline.png\" alt=\"\" title=\"" . $popupInfo . "\"/>");
			...
			echo("<table><tr><td>");
			echo("<img src=\"$host_url/teamspeakdisplay/teamspeak_online.png\" alt=\"\" title=\"" . $popupInfo . "\"/>");
			echo("</td><td class=\"teamspeakserver\" title=\"" . $popupInfo . "\">");
			echo("<a class=\"teamspeakserver\" href=\"javascript:enterServer_" . $jsTeamspeakId . "();\">");
			echo(str_replace(" ", "&nbsp;", htmlentities(substr($serverInfo["serverinfo"]["server_name"],0,$settings['maxdisplaylength']))));
			echo("</a>");
			echo("</td></tr></table>\n");
			...
			foreach($serverInfo["channellist"] as $channelInfo) { if ($channelInfo["parent"] == -1) {
				...
				if ($channelInfo["topic"] != "") { $popupInfo = $popupInfo . ", Topic: " . htmlentities($channelInfo["topic"]); }
				
				// Display channel:
				echo("<table><tr><td>");
				echo("<img src=\"$host_url/teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "3" : "2") . ".png\" alt=\"\"/>");
				echo("<img src=\"$host_url/teamspeakdisplay/channel.png\" alt=\"\" title=\"" . $popupInfo . "\"/>");
				...
				echo(str_replace(" ", "&nbsp;", htmlentities(substr($channelInfo["displayname"],0,$settings['maxdisplaylength']))));
				echo("</a>");
				echo("</td></tr></table>\n");
				...
				foreach($serverInfo["playerlist"] as $playerInfo) {
					...
					if ($playerInfo["channelid"] == $channelInfo["channelid"]) {
						...
						// Display player:
						echo("<table><tr><td>");
						echo("<img src=\"$host_url/teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "4" : "1") . ".png\" alt=\"\"/>");
						echo("<img src=\"$host_url/teamspeakdisplay/treeimage" . ((($counter_playerandsubchannels + 1) == $currentplayersandsubchannels) ? "3" : "2") . ".png\" alt=\"\"/>");
						echo("<img src=\"$host_url/teamspeakdisplay/player_" . $playerInfo["displayimage"] . ".png\" alt=\"" . $playerInfo["displayimage"] . "\" title=\"" . $popupInfo . "\"/>");
						echo("</td><td class=\"teamspeakplayer\" title=\"" . $popupInfo . "\">");
						echo(str_replace(" ", "&nbsp;", htmlentities(substr($playerInfo["displayname"],0,$settings['maxdisplaylength']))));
						echo("</td></tr></table>\n");
				...
				foreach($serverInfo["channellist"] as $subchannelInfo) { if ($subchannelInfo["parent"] == $channelInfo["channelid"]) { 
					...
					if ($subchannelInfo["topic"] != "") { $popupInfo = $popupInfo . ", Topic: " . htmlentities($subchannelInfo["topic"]); }
					...
					echo(str_replace(" ", "&nbsp;", htmlentities(substr($subchannelInfo["displayname"],0,$settings['maxdisplaylength']))));
					echo("</a>");
					echo("</td></tr></table>\n");
					...
					foreach($serverInfo["playerlist"] as $playerInfo) {
						...
						if ($playerInfo["channelid"] == $subchannelInfo["channelid"]) {
							...
							echo("<table><tr><td>");
							echo("<img src=\"$host_url/teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "4" : "1") . ".png\" alt=\"\"/>");
							echo("<img src=\"$host_url/teamspeakdisplay/treeimage" . ((($counter_playerandsubchannels + 1) == $currentplayersandsubchannels) ? "4" : "1") . ".png\" alt=\"\"/>");
							echo("<img src=\"$host_url/teamspeakdisplay/treeimage" . ((($counter_player + 1) == $currentplayers) ? "3" : "2") . ".png\" alt=\"\"/>");
							echo("<img src=\"$host_url/teamspeakdisplay/player_" . $playerInfo["displayimage"] . ".png\" alt=\"" . $playerInfo["displayimage"] . "\" title=\"" . $popupInfo . "\"/>");
							echo("</td><td class=\"teamspeakplayer\" title=\"" . $popupInfo . "\">");
							echo(str_replace(" ", "&nbsp;", htmlentities(substr($playerInfo["displayname"],0,$settings['maxdisplaylength']))));
							echo("</td></tr></table>\n");
